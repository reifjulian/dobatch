...
program define dobatch_wait, rclass
	...
	***
		...
		while (`check_cpus'==1) {
			...
			cap rm `tmp'
			if `is_windows' {
				* Convert space-separated PIDs to comma-separated for Get-Process -Id
				local pid_comma : subinstr local pid " " "," , all
				qui shell powershell -NoProfile -Command "if (Get-Process -Id `pid_comma' -ErrorAction SilentlyContinue) { '' | Set-Content '`tmp'' }"
			}
			else {
